...
void moveTrack(void) {
  ...
  int temp, leftSensorReading, middleSensorReading, rightSensorReading; // temp unused.
  ...
  while (1) {
   ...
   leftSensorReading = digitalRead(leftSensor);
   middleSensorReading = digitalRead(middleSensor);
   rightSensorReading = digitalRead(rightSensor);
    
    if ((leftSensorReading == 0) && (middleSensorReading == 0) && (rightSensorReading == 0)) {
      //leftMotor1.run(5); rightMotor1.run(5);//5-> stop
      //leftMotor2.run(5); rightMotor2.run(5);
      //leftMotor1.setSpeed(0); rightMotor1.setSpeed(0);
      //leftMotor2.setSpeed(0); rightMotor2.setSpeed(0);
      ...
      setMotorDirection(CAR_STOP);
      stopMotor();
      continue;
    } else if (leftSensorReading == 0 && rightSensorReading == 1) { //go to right
      //leftMotor1.run(3); rightMotor1.run(3);//3-> left
      //leftMotor2.run(3); rightMotor2.run(3);
      //leftMotor1.setSpeed(0); rightMotor1.setSpeed(75);
      //leftMotor2.setSpeed(0); rightMotor2.setSpeed(75);

      setMotorDirection(CAR_LEFT);
      stopLeftMotor();
      setRightMotorSpeed(HALF_SPEED);
    ...
    } else if ((rightSensorReading == 0) && (leftSensorReading == 1)) { // go left
      //leftMotor1.run(4); rightMotor1.run(4);//4-> right
      //leftMotor2.run(4); rightMotor2.run(4);
      //leftMotor1.setSpeed(75); rightMotor1.setSpeed(0);
      //leftMotor2.setSpeed(75); rightMotor2.setSpeed(0);

      setMotorDirection(CAR_RIGHT);
      setLeftMotorSpeed(HALF_SPEED);
      stopRightMotor();
    ...
    } else if(middleSensorReading == 1 && rightSensorReading == 1){
      //leftMotor1.run(5); rightMotor1.run(5);//5-> stop
      //leftMotor2.run(5); rightMotor2.run(5);      //STOP_CAR IF OFF TRACK
      //leftMotor1.setSpeed(0); rightMotor1.setSpeed(0);
      //leftMotor2.setSpeed(0); rightMotor2.setSpeed(0);
      //continue;
      ...
      neckControllerServoMotor.attach(SERVO_PIN);
      neckControllerServoMotor.write(90); //FORWARD FACING SENSOR
      
      //leftMotor1.run(1); rightMotor1.run(1);//1-> forward
      //leftMotor2.run(1); rightMotor2.run(1);
      //leftMotor1.setSpeed(75); rightMotor1.setSpeed(75);
      //leftMotor2.setSpeed(75); rightMotor2.setSpeed(75);

      setMotorDirection(CAR_FORWARD);
      setMotorSpeed(75);
      
      S = readPing();
      if(S>0 && S<=TURN_DIST){
        //leftMotor1.run(5); rightMotor1.run(5);// 5-> stop
        //leftMotor2.run(5); rightMotor2.run(5);  // IF OBSTACLE DETECTED 
        //leftMotor1.setSpeed(0); rightMotor1.setSpeed(0);
        //leftMotor2.setSpeed(0); rightMotor2.setSpeed(0);
...
/**
 * George, Justin, Vincent, and Nam
 * This function turns the car to the side of the obstacle so it doesn't collide with it.
 */
...
void turn() {
  ...
  if (Sleft <= TURN_DIST && Sright <= TURN_DIST) {
    ...
    setMotorDirection(CAR_BACKWARD);
    setMotorSpeed(200);
    delay(300);
...
/**
 * George, Justin, Vincent, and Nam
 * This function has the car go around the obstacle in a straight line until it no longer detects it.
 */
...
void navigate(void) {
  ...
  while(!around){
    ...
    if (S > 0 && S <= TURN_DIST){
      delay(30);
      shiftRight(); //SHIFT RIGHT UNTIL CORNER FOUND
    } else {
      //leftMotor1.run(5); rightMotor1.run(5);//5-> stop
      //leftMotor2.run(5); rightMotor2.run(5);
      //leftMotor1.setSpeed(0); rightMotor1.setSpeed(0);
...
/**
 * George, Justin, Vincent, and Nam
 * This function moves the car back onto the track from where it was after the turn function ends.
 */

void sidephase(void) {
  neckControllerServoMotor.write(179);
  boolean hasReachedCorner = false;
 
  while (!hasReachedCorner){
    S = readPing();
    if (S > 0 && S <= TURN_DIST){
      //leftMotor1.run(1); rightMotor1.run(1);//1-> forward
      //leftMotor2.run(1); rightMotor2.run(1);
...
/**
 * George, Justin, Vincent, and Nam
 * This function moves the car to rejoin the track after successfully going around it. 
 */

void rejoinTrack(void) {
  //leftMotor1.run(1); rightMotor1.run(1);//1-> forward
  //leftMotor2.run(1); rightMotor2.run(1);
  //leftMotor1.setSpeed(75); rightMotor1.setSpeed(75);
  //leftMotor2.setSpeed(75); rightMotor2.setSpeed(75);
  ...
  //leftMotor1.run(3); rightMotor1.run(3);//3-> left
  //leftMotor2.run(3); rightMotor2.run(3);
  //leftMotor1.setSpeed(0); rightMotor1.setSpeed(75); //getting back to the line 
  //leftMotor2.setSpeed(0); rightMotor2.setSpeed(75);
  //delay(900);
  ...
  int temp, leftSensorReading, middleSensorReading, rightSensorReading; // temp, rightSensorReading unused.
...
/**
 * Methods (Vincent) made to clean up code.
 */
 
void setLeftMotorDirection(int motorDirection) {
  leftMotor1.run(motorDirection);
  leftMotor2.run(motorDirection);
}

void setRightMotorDirection(int motorDirection) {
  rightMotor1.run(motorDirection);
  rightMotor2.run(motorDirection);
}

void setMotorDirection(int motorDirection) {
  setLeftMotorDirection(motorDirection);
  setRightMotorDirection(motorDirection);
}

void setLeftMotorSpeed(int motorSpeed) {
  leftMotor1.setSpeed(motorSpeed);
  leftMotor2.setSpeed(motorSpeed);
}

void setRightMotorSpeed(int motorSpeed) {
  rightMotor1.setSpeed(motorSpeed);
  rightMotor2.setSpeed(motorSpeed);
}
